@extends('admin.master.index')
@section('content')
<div class="container-xxl flex-grow-1 container-p-y">
   <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Dashboard /</span> Fashion Designs</h4>
   <div class="card">
      <h5 class="card-header">
      Fashion Designs
      </h5>
      <div class="table-responsive text-nowrap" style="min-height: 500px">
         <table class="table table-bordered">
            <thead>
               <tr>
                  <th>#</th>
                  <th>Image</th>
                  <th>Title</th>
                  <th>Designer</th>
                  <th>Price</th>
                  <th>Category</th>
                  <th>Actions</th>
               </tr>
            </thead>
            <tbody>
               @forelse($designers as $index=>$designer)
               <tr>
                   <td>
                       {{$index +1 }}
                   </td>
                   <td><img src="{{$designer->image_path}}" class="img-thumbnail" style="height: 100px;width: 100px;"></td>
                  <td><i class="fab fa-angular fa-lg text-danger me-3"></i> <strong>{{$designer->title}}</strong></td>
                  <td>
                     {{ $designer->designer?->name }}
                  </td>
                  <td>
                     {{ $designer->price }} SAR
                  </td>
                   <td>{{optional($designer->category)->name}}</td>
                  <td>
                     <div class="dropdown">
                        <button type="button" class="btn p-0 dropdown-toggle hide-arrow" data-bs-toggle="dropdown">
                        <i class="bx bx-dots-vertical-rounded"></i>
                        </button>
                        <div class="dropdown-menu">
                           <a class="dropdown-item" href="{{route('admin.fashions.show', $designer->id)}}">
                              <i class="bx bx-show me-1"></i>Show
                           </a>
                           <a class="dropdown-item btn-del" data-action="{{route('admin.fashions.destroy', $designer->id)}}"  data-bs-toggle="modal" data-bs-target="#modalToggle" href="#">
                              <i class="bx bx-trash me-1"></i> Delete
                           </a>
                        </div>
                     </div>
                  </td>
               </tr>
               @empty
               <tr>
                  <th colspan="4" class="text-center">No Data Found.</th>
               </tr>
               @endforelse
            </tbody>
         </table>
         <div class="text-center">
            {{ $designers->links() }}
         </div>
      </div>
   </div>
</div>
@endsection
